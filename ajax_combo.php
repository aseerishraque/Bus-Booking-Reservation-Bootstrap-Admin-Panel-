	...
	$lookups = array(   
		...
		'availability' => array(   
			...
			'route' => array(
				'parent_table' => 'routes',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`routes`.`name`) || CHAR_LENGTH(`routes`.`time`), CONCAT_WS(\'\', `routes`.`name`, \'  :\', `routes`.`time`), \'\')',
				'parent_from' => '`routes` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
		...
		'bookings' => array(   
			'id_number' => array(
				...
				'parent_pk_field' => 'id',
				'parent_caption' => '`customers`.`id_number`',
			...
			'fullname' => array(
				'parent_table' => 'customers',
				'parent_pk_field' => 'id',
				'parent_caption' => '`customers`.`fullname`',
				'parent_from' => '`customers` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'bus' => array(
				'parent_table' => 'availability',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`availability`.`bus`) || CHAR_LENGTH(`availability`.`route`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`buses1`.`number`), CONCAT_WS(\'\',   `buses1`.`number`), \'\'), \'   |:|\', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS(\'\',   `routes1`.`name`, \'  :\', `routes1`.`time`), \'\')), \'\')',
				'parent_from' => '`availability` LEFT JOIN `buses` as buses1 ON `buses1`.`id`=`availability`.`bus` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `availability`.`id`, IF(CHAR_LENGTH(`availability`.`bus`) || CHAR_LENGTH(`availability`.`route`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`buses1`.`number`), CONCAT_WS(\'\',   `buses1`.`number`), \'\'), \'   :\', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS(\'\',   `routes1`.`name`, \'  :\', `routes1`.`time`), \'\')), \'\') FROM `availability` LEFT JOIN `buses` as buses1 ON `buses1`.`id`=`availability`.`bus` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` WHERE `availability`.status=\'available\' ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => false
			),
			...
			'amount' => array(
				'parent_table' => 'availability',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS(\'\',   `routes1`.`amount`), \'\')',
				'parent_from' => '`availability` LEFT JOIN `buses` as buses1 ON `buses1`.`id`=`availability`.`bus` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'date' => array(
				'parent_table' => 'availability',
				'parent_pk_field' => 'id',
				'parent_caption' => 'if(`availability`.`date`,date_format(`availability`.`date`,\'%m/%d/%Y\'),\'\')',
				'parent_from' => '`availability` LEFT JOIN `buses` as buses1 ON `buses1`.`id`=`availability`.`bus` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'time' => array(
				'parent_table' => 'availability',
				'parent_pk_field' => 'id',
				'parent_caption' => '`availability`.`time`',
				'parent_from' => '`availability` LEFT JOIN `buses` as buses1 ON `buses1`.`id`=`availability`.`bus` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
