	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'buses' => array('Buses', '', 'resources/table_icons/yellow_submarine.png', 'None'),
			'seats' => array('Seats', '', 'resources/table_icons/chair.png', 'None'),
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'buses' => "`buses`.`id` as 'id', `buses`.`number` as 'number'",
			'seats' => "`seats`.`id` as 'id', `seats`.`name` as 'name'",
			'availability' => "`availability`.`id` as 'id', IF(    CHAR_LENGTH(`buses1`.`number`), CONCAT_WS('',   `buses1`.`number`), '') as 'bus', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS('',   `routes1`.`name`, '  :', `routes1`.`time`), '') as 'route', IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS('',   `routes1`.`amount`), '') as 'amount', if(`availability`.`date`,date_format(`availability`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`availability`.`time`, '%r') as 'time', `availability`.`status` as 'status'",
			'bookings' => "`bookings`.`id` as 'id', IF(    CHAR_LENGTH(`customers1`.`id_number`), CONCAT_WS('',   `customers1`.`id_number`), '') as 'id_number', IF(    CHAR_LENGTH(`customers1`.`fullname`), CONCAT_WS('',   `customers1`.`fullname`), '') as 'fullname', IF(    CHAR_LENGTH(`customers1`.`phone`), CONCAT_WS('',   `customers1`.`phone`), '') as 'phone', IF(    CHAR_LENGTH(`buses1`.`number`) || CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS('',   `buses1`.`number`, `routes1`.`name`, '  :', `routes1`.`time`), '') as 'bus', IF(    CHAR_LENGTH(`seats1`.`name`), CONCAT_WS('',   `seats1`.`name`), '') as 'seat', IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS('',   `routes1`.`amount`), '') as 'amount', IF(    CHAR_LENGTH(if(`availability1`.`date`,date_format(`availability1`.`date`,'%m/%d/%Y'),'')), CONCAT_WS('',   if(`availability1`.`date`,date_format(`availability1`.`date`,'%m/%d/%Y'),'')), '') as 'date', IF(    CHAR_LENGTH(`availability1`.`time`), CONCAT_WS('',   `availability1`.`time`), '') as 'time', `bookings`.`luggage` as 'luggage'",
			'routes' => "`routes`.`id` as 'id', `routes`.`name` as 'name', `routes`.`time` as 'time', `routes`.`amount` as 'amount'",
			'customers' => "`customers`.`id` as 'id', `customers`.`fullname` as 'fullname', `customers`.`phone` as 'phone', `customers`.`id_number` as 'id_number'"
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Bus_Booking_System_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Bus_Booking_System_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Bus_Booking_System_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Bus_Booking_System_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Bus Booking System</a>
			</div>
